...
| WATCH-6 | Watch safety, consent, and resource governance | Features | M | In progress | Networked and AI watch actions stay opt-in per profile under the no-silent-network promise (AI-5): nothing reaches a cloud without explicit per-profile consent, and provider, model, and scope are shown. Wall-clock and concurrency caps (shared with the Python sandbox limits, SEC-9) prevent a runaway folder from overwhelming the machine; termination is announced. A dry-run preview shows exactly what a profile would do before it is armed. The atomic claim from WATCH-3 prevents double-processing. Tests cover the consent gate, the resource cap, dry-run, and the claim. The `core` layer now enforces the consent gate (`WatchActionRegistry.run` returns an announced `skipped` outcome for any `requires_consent` action — currently the AI action — until the profile carries explicit `options["consent"] is True`) and exposes a side-effect-free `WatchActionRegistry.dry_run(...)` that returns a plain-language preview of what a profile would do (or the reason it would not: unknown action, disabled feature, invalid options, or missing consent). The sandboxed Python transform action honors a clamped wall-clock timeout shared with SEC-9, and the single-threaded worker already serializes processing (concurrency of one). Tests cover the consent gate (skipped without consent, runs with consent), dry-run (runnable preview, invalid options, missing consent, unknown action). Remaining: the per-profile consent UI that shows provider/model/scope before arming, surfacing the dry-run preview in the profile manager, and announcing resource-cap termination. |
| WATCH-7 | Built-in watch actions | Features | M | In progress | Ship the core action set on top of WATCH-2: Open, Convert or export to a chosen format (via the existing IO and Pandoc paths), Move/copy/archive to a destination, Run a named macro (FEAT-7), Run a saved Python transform (sandboxed, SEC-9), and consented AI actions (summarize, tag, rewrite to a style) via the assistant. Each action validates its options, announces its outcome, and routes failures to the queue's failed state with a clear reason. Tests cover each action's happy path and its failure path. The full action set now ships in `quill/core/watch_actions.py` and registers through `default_registry`: `OpenAction`, `MoveAction`, the new `CopyAction` (copy to a destination, original left in place), `ConvertAction` (export to a chosen format via a caller-supplied IO/Pandoc handler), `RunMacroAction` (open and replay a named macro via a caller-supplied editor handler), `RunPythonTransformAction` (read the file's text, run a saved transform through the SEC-9 sandbox with a clamped timeout, write the result in place or to a suffixed file), and the consent-gated `AiAction` (summarize/tag/rewrite via a caller-supplied handler, refused until per-profile consent). Each validates its options, returns `done`/`failed` outcomes that route through the queue's lifecycle, and offers a `preview` for dry-run (WATCH-6). Open, Move, Copy, and the sandboxed Python transform are fully functional; Convert, Run macro, and consented AI are implemented and unit-tested at the registry seam and wired through `WatchService`/`default_registry`, awaiting the UI to supply their handlers (the same callback pattern `OpenAction` already uses). Tests cover each action's happy and failure paths (copy, convert with/without handler, macro, Python transform success/failure/suffixed output, AI consent gate and modes). |
...
| Tier 2 | Flagship experience | 58 | 29 | 29 | OCR-1..5, AGENT-1, AI-7, AI-1, AI-6, AI-14, AI-19, AI-20, AI-22, AI-24, SET-1..4, SET-7, SHARE-1..2, CTX-1, DICT-2, FEAT-19, WATCH-5..7, FLAG-3, DLG-1 |
...
| --- | --- | --- |
| Tier 2 — Flagship | In progress | SET-1, SET-2, SET-3, SET-4, SET-7, SHARE-1, SHARE-2, WATCH-5, WATCH-6, WATCH-7, MENU-1, MENU-5, AGENT-1 |
...
| Tier 2 — Flagship | QK-1, QK-2, QK-3, QK-4, QK-5, QK-9, NAV-1, NAV-4, NAV-5, SEL-1, SEL-2, SEL-3, AI-13, AI-15, AI-16, AI-17, AI-21, AI-23, WATCH-1, WATCH-2, WATCH-3, WATCH-4, SET-5, SET-6, SHARE-3, FLAG-4, MENU-3, DICT-1 |
