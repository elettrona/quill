...
- Status: **Shipped** — merged to `main` as PR #987 (0.9.0 Beta 3).
- Created: 2026-07-12
...
- `favorites.py` — saved stations (from any source) as atomic JSON, the standard QUILL settings-surface pattern. Each favorite carries an optional `folder` field, unused by radio today but there on purpose so podcasts (`docs/planning/podcasts.md` §1's `PodcastFolder`) could have grown out of the same shape — in practice podcasts ended up with a richer, separate nested-folder model of its own, but the flat `folder` field is harmless to leave as-is here.
...
## 3. Podcasts — moved to its own spec

The original lighter sketch that lived here (folders of shows, OPML import/export, playlists from a folder, discovery/reader design questions) has been superseded by a full brainstorming pass and now lives in **`docs/planning/podcasts.md`** — a much larger, richer spec (data model, Inbox vs. library folders, Play Queue, resume position + cross-device sync, transcripts, local podcasts, and more). That is now the canonical Podcasts planning document; nothing podcast-specific should be added here going forward.
...
- **Sleep timer** (stop after N minutes, or at a natural stopping point) — added to the Podcasts spec (`docs/planning/podcasts.md` §5) during that design pass; Radio should get the same control on its PlayerController/status bar for parity, since "listening to fall asleep" applies to both.
