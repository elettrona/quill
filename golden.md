...
| OPS-1 | Green main: required CI gates must pass | Foundations | S | In progress | The gate ladder is only real if it is green on `main`. The newly added gate jobs were red because they installed only `.[dev]`: the UI test modules need `wx` (the `ui` extra) and the scoped-typing job could not find optional modules (`markitdown`, `certifi`, `pdfplumber`, `pyttsx3`). Fix: install `.[dev,ui]` in the test jobs and add the optional modules to the mypy `ignore_missing_imports` overrides so the typing gate measures first-party code regardless of optional extras. Acceptance: PR CI, Security CI, and Accessibility CI all conclude success on `main`. This is the first thing to resolve before any further work. |
...
| --- | --- | --- | --- | --- | --- |
| Tier 1 | Protect users and unlock the team | 23 | 20 | 3 | OPS-1 (in progress), FLAG-1, FLAG-2 |
