...
## Durable Publishing Linkage Registry Complete - 2026-06-22

The first of the two decisions left open at roadmap close is resolved. A previously-linked, previously-saved publishing document now keeps its Compare/Update/Schedule link after being closed and reopened (or after the app restarts) — `quill/core/publishing_linkage.py` persists the linkage in a path-keyed JSON store under the app data directory, and `main_frame.py` saves to it on every save and successful publish/update/schedule, and restores from it on every open. Untitled documents and CSV/Word structured-surface tabs are deliberately excluded, since Compare/Update were never designed to work against them. The remaining open decision is unchanged: whether/when to loosen `core.third_party_plugins`/SEC-8 for real third-party Quillin or publishing-provider execution.