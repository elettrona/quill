    ...
    def _refresh_watch_queue_monitor(self) -> None:
        ...
        if pause_button is not None:
            pause_button.SetLabel(
                "Resume" if self._watch_service.queue.is_paused() else "Pause"
            )
    ...
    def _edit_watch_profile(self, profile: WatchProfile | None) -> WatchProfile | None:  # noqa: PLR0915
        ...
        with wx.Dialog(self.frame, title=title) as dialog:
            ...
            interval_input = wx.SpinCtrl(
                panel, min=2, max=300, initial=base.poll_interval_seconds
            )
            ...
            post_action = post_values[post_index] if 0 <= post_index < len(post_values) else POST_LEAVE
