...
## 2026-06-21 Live Third-Party Provider Loading — Contract Shipped, Loading Deliberately Not Implemented

Research before implementation found this phase is not a publishing-scoped engineering task at heart: SEC-8 (`docs/QUILL-PRD.md`) is a product-wide lock — the `core.third_party_plugins` feature flag (`locked_off=True`, `quill/core/feature_catalog.py`) keeps *all* third-party Quillin code from executing in a default QUILL 1.0 build, not just publishing-related code. "Install from Folder" already exists and is *not* gated by that flag, but the code still never runs while the flag is locked off. Critically, `register_publishing_provider`/`register_publishing_provider_client` (`quill/core/publishing_providers.py`, `quill/core/publishing_clients.py`) are completely open, ungated functions today — any in-process code could already call them directly, bypassing even the Phase 1 bundled-adapter validation contract. So "live third-party provider loading" is really asking whether to create an exception to, or flip, a documented security boundary that protects far more than publishing — a product policy decision, not an implementation detail to infer.

Presented the user three explicit options (build the contract with loading kept locked off; actually flip the SEC-8 flag and wire real loading; stop here as policy-blocked). The user chose the first, recommended option — the same "build the contract, defer the risky part" pattern used in every prior phase of this roadmap.

Implemented: `ThirdPartyPublishingProviderAdapter` and `register_third_party_publishing_provider` in `quill/core/publishing_adapters.py`. Unlike the bundled adapter, `secret_access` and `execution` have no trusted defaults — a third-party adapter must state its policy explicitly. Validation cascade: ids must match; network rationale required; secrets must be host-owned; execution must be `worker` (in-process is rejected outright — untrusted code never gets the trusted path); provider id must not conflict with an existing bundled adapter. Every one of those branches is independently tested and provably enforced. The function's *last* line, reached only by a fully well-formed adapter, unconditionally raises "Live third-party publishing provider loading is not implemented yet." — proven by a dedicated test that the live registries (`PROVIDER_DEFINITIONS`, `_PUBLISHING_PROVIDER_CLIENTS`) are untouched even in that case. The module docstring states explicitly that this contract does not loosen, bypass, or duplicate the `core.third_party_plugins`/SEC-8 lock, and that implementing real loading remains a separate, explicit product decision.

Validation: focused battery `82 passed` (14 new adapter tests); Ruff, mypy (unchanged pre-existing findings), and the provider registry gate passed (confirming zero registry impact — nothing third-party ever reaches it); full unit suite `4100 passed, 66 failed, 14 skipped` — identical pre-existing failure set, +7 passing delta matching the new tests. No UI, dialog, menu, or module-size-budget changes — `publishing_adapters.py` stayed at 146 lines, well under the untracked cap.

Committed locally as one checkpoint; not pushed pending explicit request.

## Roadmap Closed

All four phases the 2026-06-20 Phase 1 closeout authorized are now addressed: (1) WordPress first-party bundled-provider path, (2) schedule publishing, (3) local-versus-remote compare, (4) Quillin worker execution boundaries / live third-party provider loading — the last two converged on the same conclusion: build the contract, defer the boundary, because no untrusted code can run in this product yet. Two explicit, separate product decisions remain open for the user/product owner outside of implementation work: whether to ever build the durable cross-session publishing-linkage registry deferred in the compare phase, and whether/when to loosen `core.third_party_plugins` (SEC-8) for real third-party Quillin and publishing-provider execution. Neither is an engineering default to assume.

## 2026-06-22 Durable Publishing Linkage Registry Implemented (First Open Decision Resolved)

The user chose to resolve the first of the two decisions left open at roadmap close: building the durable, file-path-keyed publishing-linkage registry deferred by the compare phase. Implemented as a Claude Code session, planned via `EnterPlanMode`/`ExitPlanMode` after direct code research (no Explore agent needed — the relevant call sites were small enough to read directly).

Confirmed by reading the actual code: `Document.source_metadata` (`quill/core/document.py`) carries ten `publishing_*` fields plus two guard keys (`source_kind`, `source_label`), set in exactly two main_frame.py sites (browse-and-open, send-remote-item-success) and read in two more (compare, send). `Document.name` falls back to `display_name` only when `path is None`, so restoring linkage on reopen never needs to touch `display_name`. The save chokepoint is `_write_document_to_disk` (called from `save_file`/`save_file_as` only); the format-specific writers in `quill/io/export.py` call `document.mark_saved(target)` before returning, so `document.path` is guaranteed set by the time the chokepoint returns. The open chokepoint is `_finish_open_document`, reached by every open including `_reload_in_place`'s Save-As-then-reload-in-matching-view flow.

Implemented:

- New `quill/core/publishing_linkage.py`: `PublishingLinkageEntry` frozen dataclass, a path-keyed JSON store at `app_data_dir() / "publishing-linkage.json"` (mirrors the existing `publishing-connections.json` pattern exactly — `read_json`/`write_json_atomic`, defensive `isinstance` checks on every level of the loaded JSON), and `publishing_linkage_from_source_metadata`/`apply_publishing_linkage_to_source_metadata` converters that round-trip against `Document.source_metadata` without ever touching unrelated keys already in that dict.
- `MainFrame._sync_publishing_linkage_for_document`: the single helper both the save chokepoint and every successful-publish call site (`_send_publishing_remote_item`, the schedule-publish handler, and the create-draft/publish-now handler — three sites, not the one originally scoped in the plan; the other two were discovered while implementing and added for consistency) call after refreshing `source_metadata`. Skips when `document.path is None` (untitled documents are never registered) and when the active editor is a `CsvGridSurface`/`WordDocumentSurface` (Compare/Update only ever read `self.editor.GetValue()` as markdown/HTML text, a shape those structured surfaces were never designed to produce — linkage is never persisted for them, so it can never be wrongly restored either).
- `_finish_open_document` looks up the registry by `selected_path` and merges a hit into the freshly-read document's `source_metadata` before either the new-tab or refresh-existing branch runs.

Real bug caught during implementation, not just typed and shipped: the first version of `_sync_publishing_linkage_for_document` did bare `self.editor`/`document.source_metadata` attribute access and broke an existing characterization test (`test_main_frame_cq16_characterization.py::test_write_document_to_disk_routes_rtf_through_the_rtf_writer`), which calls `_write_document_to_disk` against a bare `MainFrame.__new__` test double with no `editor` attribute and a `SimpleNamespace(path=...)` document with no `source_metadata`. That characterization suite's own stated convention is "stub only the attributes each method touches," matching the pre-existing `getattr`-defensive style already used a few lines above for `self.settings`. Rewrote the helper to be fully `getattr`-defensive (checks `source_metadata` is a dict before doing anything else, so the common non-publishing-document case never even looks at `self.editor`), confirmed via re-running the full suite.

Validation methodology note for future sessions: full-suite comparisons in this slice were run with pytest's *default* temp directory, not a custom `--basetemp`. Earlier phases in this roadmap (schedule/compare/worker-execution/third-party-loading) validated against `--basetemp=.tmp\pytest-<slice>-full`, a path under the repo and therefore outside `Path.home()`. `quill/core/paths.py`'s H-1-core guard (`_is_constrained_to_home`) silently rejects a `QUILL_DATA_DIR` override that isn't under home, so any test relying on the shared `quill_data_dir` conftest fixture (which does not itself override `Path.home`) would have silently fallen through to the *real* `%APPDATA%\Quill` directory under that custom basetemp — breaking isolation and very plausibly explaining why this roadmap's recorded baseline was `66 failed` rather than the `19 failed` this slice independently verified via `git stash`/clean-tree comparison (identical 19 failure names with and without this slice's changes; none publishing-, document-, or export/open_read-related). Not re-litigated or fixed retroactively — flagging only so a future session does not assume 66 is the true baseline, and does not repeat the same custom-basetemp pattern for isolation-sensitive tests. One concrete, minor side effect from this discovery: this slice's own first (buggy) test run wrote one bogus entry to the real `%APPDATA%\Roaming\Quill\publishing-linkage.json` before the fixture was fixed; the user was asked and chose to leave it (harmless test data, not real app data).

Validation: focused battery `77 passed` (linkage core tests, publishing/compare core tests, main_frame tests including the CQ-16 characterization suite, module-size budget tests); Ruff and `ruff format --check` passed; scoped `mypy quill/core quill/io` unchanged (same 7 pre-existing findings); full unit suite `4167 passed, 19 failed, 14 skipped`, with the 19 failures verified pre-existing and unrelated via a clean-tree (`git stash`) comparison showing the identical 19 names at `4147 passed`. `main_frame.py`'s module-size budget bumped 25127->25165 (+38) with a dated rebaseline entry.
