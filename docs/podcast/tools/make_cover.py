...
def _soundwave(size: int, y: float, bars: int, bar_w: int, gap: int,
               max_h: int, colors: list[tuple[int, int, int]]) -> Image.Image:
    ...
    for i in range(bars):
        ...
        draw.rounded_rectangle((x, y - h / 2, x + bar_w, y + h / 2), radius=bar_w // 2, fill=(*col, 255))
    return layer


def _centered(draw: ImageDraw.ImageDraw, text: str, font: ImageFont.FreeTypeFont, y: int,
              fill, *, letter_spacing: int = 0) -> int:
...
def main() -> int:
    ...
    img.alpha_composite(_soundwave(SIZE, y=1760, bars=72, bar_w=34, gap=18,
                                   max_h=560, colors=[TEAL, GOLD, PINK]))
    ...
    _centered(draw, "community-access.github.io/quill", _font(58), y=2760, fill=DIM, letter_spacing=6)
