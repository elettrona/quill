...
persisted default. `RecordingScheduler` is an in-process polling thread
...
`link.acb.org`), a working mpv backend for live streams (needed before
gapless/exact-seek playback can apply to Radio, not just Podcasts), ICY
"now playing" metadata, a live-refresh fetcher for the currently
static/hand-maintained ACB Media station list, and optional OS-level
global hotkeys remain — see `docs/planning/roadmap.md` §1.7.
...
the 5-phase plan in `docs/planning/podcasts.md` (discovery, subscriptions,
...
Search Everywhere) — see `docs/planning/podcasts.md` for the full phased
plan.
