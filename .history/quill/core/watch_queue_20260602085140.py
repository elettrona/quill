...
_ALL_STATES = frozenset(
    {STATE_QUEUED, STATE_PROCESSING, STATE_DONE, STATE_FAILED, STATE_SKIPPED}
)
...
class WatchQueue:
    ...
    def mark_failed(self, item_id: str, message: str = "") -> None:
        ...
        with self._lock:
            ...
            if item.attempts < self._max_attempts:
                backoff = min(
                    self._base_backoff * (2 ** max(0, item.attempts - 1)), _MAX_BACKOFF
                )
                requeued = replace(
                    item,
                    state=STATE_QUEUED,
                ...
                return
            failed = replace(
                item, state=STATE_FAILED, updated_at=now, message=message
            )
    ...
    def retry(self, item_id: str) -> bool:
        ...
        with self._lock:
            ...
            if item is None or item.state != STATE_FAILED:
                return False
            requeued = replace(
                item, state=STATE_QUEUED, updated_at=now, not_before=now, message=""
            )
            self._items[item_id] = requeued
            self._save_locked()
    ...
    def pending_count(self) -> int:
        with self._lock:
            return sum(
                1 for item in self._items.values() if item.state == STATE_QUEUED
            )
...
# Keep ``field`` import meaningful for static analysers that flag unused names.
_ = field
