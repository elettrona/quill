...
- **Tab key can now insert a literal tab character (Ctrl+Alt+M).** The Tab key still defaults to Quill's smart line indent, but a new session toggle switches it to insert a tab character at the caret like a plain text editor. Toggle it with **Ctrl+Alt+M** (Ctrl+M and Ctrl+Shift+M are taken by the mark ring), via the checkable **Format → Tab Key Inserts Tab Character** menu item, or by activating the new **Tab Mode** status-bar cell (shows **Indent** / **Tab char**). The mode change is spoken. Shift+Tab continues to outdent even in tab-character mode. New `format.toggle_tab_insert_mode` command bound in both keymap profiles.
