...
- **Locked Dictation now sounds distinct, and a one-time hint explains the keys.** Locked Dictation plays its own **distinct earcons** (`dictation_locked_on` / `dictation_locked_off`, new in the bundled Ink pack) instead of sharing the Hold-to-Dictate transcription cues, so a sustained hands-free session is audibly different from a press-and-hold one. The first time you start dictation, QUILL speaks a **one-time onboarding hint** (the F9 / Ctrl+F9 keys and Escape behavior) and remembers not to repeat it (`dictation_onboarding_shown` setting).
...
### Offline voice commands (Speech S5)

- **Drive QUILL hands-free, offline, with spoken commands (#663).** New **AI > Speech > Voice Command (Offline)** (`tools.voice_command`) is push-to-talk: run it, speak one command ("save", "bold", "next heading", "word count", "command palette", …), run it again to act. Recognition uses the on-device `quill/core/speech` stack (whisper.cpp) — no cloud, no upload. The set of commands voice can invoke is the **agent safe-tool allowlist** (`SAFE_TOOL_IDS`), so a misheard phrase can never trigger a destructive action; dispatch double-checks the allowlist before running. Off by default (`settings.voice_commands_enabled`) and always disabled in Safe Mode, matching the assistant/agent guarantees. The matcher (`quill/core/speech/voice_commands.py`, wx-free) is exact/alias/sub-sequence based with a cancel phrase ("cancel" / "never mind") and a spoken "no command matched; heard: …" when nothing fits. Tests: `test_voice_commands.py` (16 cases) + `test_voice_command_wiring.py` (5). Closes #663 (Speech S5). **Live mic/recognition behavior warrants a manual smoke run.**
...
- **Offline speech & dictation plan (#617).** Grounded the offline speech-to-text provider architecture in the current code (at the time, dictation only shelled out to Windows dictation; the vosk/whisper settings were dead stubs) and laid out a bold-but-simple reengineering: one offline-first speech engine, two user verbs (Dictate / Transcribe), an accessible model manager, captions, and gated voice commands that reuse the agent `SAFE_TOOL_IDS` allowlist.
