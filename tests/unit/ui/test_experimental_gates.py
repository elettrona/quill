...
* The editor-surface options additionally require the secondary
  editor-surfaces acknowledgement before any override is applied.
...
* On the Experimental tab, an unchecked master disables (and removes from the
  tab order) every other control; an unchecked surfaces gate disables the
  surface choice, border option, and explainer.
"""
...
def test_every_experimental_gate_defaults_off() -> None:
    ...
    assert settings.experimental_editor_surfaces_enabled is False
...
def test_experimental_gates_round_trip(tmp_path: Path, monkeypatch) -> None:
    ...
    settings.experimental_editor_surfaces_enabled = True
    ...
    assert loaded.experimental_editor_surfaces_enabled is True
...
def test_experimental_tab_specs_present_and_ordered() -> None:
    ...
    # The master switch leads the tab; the surfaces gate precedes the surface
    # options it governs.
    assert keys[0] == "experimental_acknowledged"
    assert keys.index("experimental_editor_surfaces_enabled") < keys.index(
        "experimental_editor_surface"
    )
    assert keys.index("experimental_editor_surfaces_enabled") < keys.index("editor_hide_border")
...
def test_surfaces_gate_carries_the_editor_impact_language() -> None:
    gate = next(
        spec for spec in SETTING_SPECS if spec.key == "experimental_editor_surfaces_enabled"
    )
    assert "degrade" in gate.label.lower()
    assert "control" in gate.label.lower()
...
def test_editor_surface_override_requires_both_gates() -> None:
    source = _main_frame_source()
    marker = source.index("Experimental overrides apply only once the user has ticked BOTH")
    window = source[marker : marker + 700]
    assert "experimental_acknowledged" in window
    assert "experimental_editor_surfaces_enabled" in window
...
def test_experimental_tab_wires_live_enable_disable_gating() -> None:
    ...
    body = source[start:end]
    # Master governs every other control; surfaces gate governs the surface set.
    for key in (
    ...
    assert "Enable(surfaces_on)" in body
