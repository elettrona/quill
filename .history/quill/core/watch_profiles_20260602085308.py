...
class WatchProfile:
    ...
    def normalized(self) -> WatchProfile:
        ...
        suffixes = tuple(
            dict.fromkeys(s for s in (_clean_suffix(x) for x in self.suffixes) if s)
        )
...
class WatchManager:
    ...
    def _poll_loop(self, profile: WatchProfile, stop_event: threading.Event) -> None:
        if not profile.process_existing:
            # Pre-seed the queue's de-dup set so existing files are ignored until
            # they change, without enqueuing them.
            try:
                for path in iter_matching_files(profile):
                    self._queue.enqueue(path, profile.profile_id, profile.action_id)
                    self._queue_skip_existing(path)
    ...
    def _queue_skip_existing(self, path: Path) -> None:
        """Immediately resolve a pre-existing file so it is not actioned.

        ``process_existing`` is off, so a file present at startup is recorded in
        the queue (claiming its de-dup slot) and marked skipped, which keeps the
        monitor honest while ensuring it is never opened or moved.
        """
        try:
        ...
        for item in self._queue.items():
            if item.source_path == canonical and item.state == "queued":
                self._queue.mark_skipped(item.item_id, "Existing file at startup")
                return
