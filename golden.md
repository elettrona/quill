...
| SHELL-2 | Structured-Markdown OCR verb (AI structuring pass) | Integration | M | In progress | The `ocr-structured` verb, its `shell_verb_ocr_structured` setting, and its AI/assistant gating are wired through the registry, IPC, CLI, and `_handle_shell_request` (it currently runs the same offline OCR pipeline as the plain verb). The actual AI pass that turns recognized text into structured Markdown is the remaining piece and needs a live assistant/AI key to verify end to end, so this stays honestly In progress rather than Done. |
| SHELL-3 | Windows 11 modern context menu (IExplorerCommand) + installer registration | Integration | M | Todo | The classic (pre-Win11) `HKCU\Software\Classes\SystemFileAssociations` verb path ships in SHELL-1 and is buildable and verifiable locally. The packaged `IExplorerCommand` sparse-package that surfaces the same verbs in the Windows 11 primary (non-"Show more options") menu, and its installer/uninstaller registration, require a live install/uninstall verification cycle and are deferred. Tracks intake issues #113 (umbrella), #114 (Windows Explorer), and #116 (structured OCR data model, delivered by SHELL-1/SHELL-2); macOS Finder (#115) stays blocked on the macOS port (#42). |
