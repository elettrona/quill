...
- **New optional offline engine: NVIDIA Parakeet (English) (#669).** A third on-device speech engine alongside whisper.cpp and Faster Whisper, for high-accuracy **English** transcription with word/segment timestamps. It is opt-in (`pip install -e ".[parakeet]"`, which pulls in NVIDIA NeMo + PyTorch — large and GPU-oriented) and, like Faster Whisper, never imported at startup: the registry probes for it with `importlib.util.find_spec` and only offers it when installed, so it can't slow launch or affect the default engine. Two pinned models (`parakeet-tdt-0.6b-v2`, `parakeet-tdt-1.1b`, both CC-BY-4.0) download from Hugging Face via **Manage Speech Models**; audio is transcoded to 16 kHz mono WAV with the bundled-or-system ffmpeg path. It is a fully **offline** provider, so the offline Watch Folder transcribe action can use it. New: `quill/core/speech/providers/parakeet.py` + catalog entries; tests in `test_parakeet_provider.py`.
- **New optional offline engine: Vosk (English, very low resource) (#669).** A fourth on-device engine for old or constrained machines: Vosk (Kaldi) is **CPU-only** and runs on a ~40 MB English model with no GPU — the accessibility-reach option where the heavier engines are impractical. Because its wheel is self-contained (it ships `libvosk`), it now **bundles in the Windows installer** (added to the build's base dependency groups), so installer users get it out of the box — no separate component or native staging, unlike whisper.cpp; from source it is the `vosk` extra (`pip install -e ".[vosk]"`). Like the others it is never imported at startup. Parakeet (NeMo + PyTorch) stays pip/source-only and is not bundled. Two Apache-2.0 models (`vosk-model-small-en-us-0.15` ~40 MB, `vosk-model-en-us-0.22` ~1.8 GB) download from alphacephei.com via **Manage Speech Models** over verified HTTPS, **MD5-pinned** and zip-slip-guarded on extract; the new outbound host is recorded in the network-egress audit. Word-level timestamps are aggregated into segments. Fully **offline**, so the offline Watch Folder transcribe action can use it. New: `quill/core/speech/providers/vosk.py` + catalog entries; tests in `test_vosk_provider.py`.
