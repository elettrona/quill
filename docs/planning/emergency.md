...
9. ◐ **Structured List Studio — Phase 2+ follow-ups** (PRD kept;
   ...
   shows nesting depth and gates controls structurally) and **multiple terms per
   entry** (one synonym per line → multiple `<dt>`). *Remaining:* multiple
   *definitions* editing, import-from-file/clipboard with preview, conversion
   previews + information-loss confirmations, in-place editing of the list under the
   caret, reparse-and-validate before commit, and the Settings/preset surface.
   *(This is list structure, not the excluded table work.)*
